...
function calculateMarketMetrics(marketData, historicalData = {}) {
  const binance4h = marketData.Binance?.["4h"] || {};
  const binance1d = marketData.Binance?.["1d"] || {};
  const bybit4h = marketData.Bybit?.["4h"] || {};
  const bybit1d = marketData.Bybit?.["1d"] || {};
  
  // Historical data for technical analysis
  const priceHistory = historicalData.priceHistory || [];
  const oiHistory = historicalData.oiHistory || [];
  const fundingHistory = historicalData.fundingHistory || [];
